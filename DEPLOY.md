...
**Важно:** Frontend использует относительные пути для API запросов (`/api/...`), которые автоматически проксируются через Nginx на backend сервер. Это означает, что **не нужно** настраивать `VITE_API_URL` на продакшене.

Для локальной разработки (если не используете прокси) можно создать файл `.env` в корне проекта:
```env
...
Если вы видите ошибку `ERR_CONNECTION_REFUSED` при обращении к `localhost:3001`, это означает, что:

1. **На продакшене:** Frontend должен использовать относительные пути через Nginx proxy. Убедитесь, что:
   - Nginx настроен и запущен
   - Конфигурация Nginx содержит блок `location /api` с `proxy_pass http://localhost:3001`
   - Backend сервер запущен на порту 3001
   - Проект пересобран после изменений (`npm run build`)

2. **Для локальной разработки:** Создайте файл `.env` в корне проекта:
   ```env
   VITE_API_URL=http://localhost:3001
   ```
   И перезапустите dev-сервер.

3. **Проверьте работу backend:**
   ```bash
